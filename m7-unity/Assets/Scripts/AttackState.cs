        ...
        if (actualTimeBetweenShoots > myEnemy.timeBetweenShoots)
        {
            ...
            if (Physics.Raycast(ray, out rayHit, 20f))
            {
                if (rayHit.transform.tag == "Player")
                {
                    ...
                    // Calcula una posición frente al enemigo para evitar colisiones
                    Vector3 spawnPosition = myEnemy.transform.position + myEnemy.transform.forward * 1.5f; // Mueve la bala 1.5 unidades al frente
